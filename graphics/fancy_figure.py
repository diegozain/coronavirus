...
class fancy_figure():
    ...
    def title_label(self,ax,fig):
        ...
        if self.super_title is not None:
            fig.suptitle(self.super_title,fontweight='bold',fontsize=20)
    @staticmethod
    def title_label_dates(self,ax,fig):
        from matplotlib.ticker import FormatStrFormatter
    ...
    def cute_ticks(self,ax):
        if isinstance(self.yticks, list):
            ...
            ax.set_yticklabels(self.yticks,fontsize='10')
    ...
    def save_me(self,fig):
        ...
        if type(self.guarda) is int:
            ...
            bbox_inches='tight',transparent=self.transparent,pad_inches=0)
    # --------------------------------------------------------------------------
    def matrix(self):
    ...
    def vline(self):
        ...
        if self.ax_ is None:
            ...
            ax = self.ax_
        # ----------------------------------------------------------------------
        ax.axvline(x=self.x,ymin=self.ymin,ymax=self.ymax,color=self.colop,linestyle=self.linestyle,markersize=self.markersize)
        # ----------------------------------------------------------------------
        ...
        # title and labels
        # .................
        self.title_label_dates(self,ax,fig)
        # .................
        # legends
        # .................
    ...
    def vspan(self):
        ...
        if self.ax_ is None:
            ...
            ax = self.ax_
        # ----------------------------------------------------------------------
        ax.axvspan(xmin=self.xmin,xmax=self.xmax,ymin=self.ymin,ymax=self.ymax,color=self.colop)
        # ----------------------------------------------------------------------
        if self.xdir == 'reverse':
            ax.invert_xaxis()
        if self.ydir == 'reverse':
    ...
    def plotter(self):
        ...
        return plt
    # --------------------------------------------------------------------------
    def plotter_date(self):
        '''
        -----------------------------
        plot a curve in 2d with time values
        -----------------------------
        simple example:
            fancy_figure(curve=y,x=x
            xlabel='x value',ylabel='y depends on x',
        ...
        if self.ax_ is None:
            ...
            ax = self.ax_
        # ----------------------------------------------------------------------
        ax.plot_date(self.x,self.curve,self.symbol, color=self.colop,markersize=self.markersize)
        # ----------------------------------------------------------------------
        if self.xdir == 'reverse':
            ax.invert_xaxis()
        if self.ydir == 'reverse':
